  ...
  private class GestionnaireEvenement extends MouseAdapter {
    ...
    public void mouseClicked(MouseEvent eve) {
      ...
      if (eve.getSource() == boutonDebuter) {
        // initialise le champ texte, apelle la mŽthode dŽbuter, et initialise
        // toute les variables
        ...
        while (increment >= -1) {
          for (int ctr = 0; ctr <= 7; ctr++) {
            tab[ctr][ligne].setIcon(new ImageIcon(
                dossierIcone + ordrePiece[ctr] + couleur + ".gif"));
            ...
            tab[ctr][ligne + increment]
                .setIcon(new ImageIcon(dossierIcone + 'P' + couleur + ".gif"));
            e.getCase(ctr, ligne + increment)
      ...
      else if (eve.getSource() instanceof JLabel) // donc on a cliqué sur un
                                                  // Label
      {
        for (int i = 0; i < 8; i++)
        ...
        if ((e.getCase(colonneClic, ligneClic).getPiece() != null
            | pieceTampon != null)) {
          // si le tampon est null
          if (pieceTampon == null) {
            // si c'est au tour de la couleur de controle ˆ jouer
            if (e.getCase(colonneClic, ligneClic).getPiece().getCouleur()
                .equals(couleurControle)) {
              // J'initialise la piece tampon a la piece sur laquelle on a
              // cliquŽ
              ...
              tab[colonneClic][ligneClic].setBorder(
                  BorderFactory.createLineBorder(new Color(255, 0, 0), 5));
            }

          } else {
            // je crŽŽ un dŽplacement
            Deplacement deplacement = new Deplacement(temp,
                new Position(colonneClic, ligneClic));
            // je vŽrifie si le dŽplacement est valide, si le chemin est
            // possible et si il est possible, pour un pion de manger la piece
            if ((pieceTampon.estValide(deplacement)
                && e.cheminPossible(deplacement))
              ...
              /*
               * je vŽrifie si la pice manger est un roi, si oui le jeu est
               * terminŽ et L'utilisateurs peut choisir si il veut continuer a
               * jouer ou non
               */
              if (e.getCase(colonneClic, ligneClic).getPiece() instanceof Roi) {
                if (JOptionPane.showConfirmDialog(null,
                    "FŽlicitation vous avez gagnŽ ! DŽsirez-vous jouer de nouveau ?\n",
                    "Mine !", JOptionPane.YES_NO_OPTION) == 0) {
                  RAZ();
                  tab[temp.getColonne()][temp.getLigne()].setBorder(
